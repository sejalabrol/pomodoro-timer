...
# example - 245 secs -> round(245/60) mins, 245%60 secs
...
def count_down(count):
    ...
    count_sec = count % 60
    if count_sec < 10:
        count_sec = f"0{count_sec}"  # dynamic typing - changing type from int to str

    canvas.itemconfig(timer_text, text=f"{count_min}:{count_sec}")
    if count > 0:
        global timer
        timer = window.after(1000, count_down, count - 1)  # milliseconds
    else:
        start_timer()
        marks = ""
...
timer_heading_label = Label(text="Timer", fg=GREEN, bg=YELLOW, font=(FONT_NAME, 35))
